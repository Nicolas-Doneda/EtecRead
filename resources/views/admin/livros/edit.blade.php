...
<div class="max-w-4xl mx-auto px-6 pt-12 pb-24">
    ...
    <div class="mb-12">
        ...
        <h1 class="text-4xl font-extrabold tracking-tight text-gray-900 mb-2">Editar Livro</h1>
        <p class="text-lg text-gray-500">{{ $livro->title }}</p>
    </div>

    <!-- Erros -->
    @if($errors->any())
        <div class="mb-8 bg-red-50 text-red-800 p-4 rounded-lg text-sm font-medium border border-red-100">
            <p class="mb-2">Erro ao atualizar obra:</p>
    ...
    <!-- Formulário -->
    <form method="POST" action="{{ route('admin.livros.update', $livro->id) }}" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="flex items-center gap-6">
                <div class="w-32 h-44 bg-gray-100 rounded-lg flex-shrink-0 flex items-center justify-center overflow-hidden border border-gray-200">
                    @if($livro->cover_image)
                        <img id="preview" src="{{ asset('storage/' . $livro->cover_image) }}" alt="{{ $livro->title }}" class="w-full h-full object-cover">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-400 mt-2">Formatos: JPG, PNG ou WEBP. Máx: 2MB. Deixe em branco para manter a atual.</p>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <input type="number" name="year" value="{{ old('year', $livro->year) }}" min="1000" max="{{ date('Y') }}"
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Estoque Total *</label>
                        <input type="number" name="total_quantity" value="{{ old('total_quantity', $livro->total_quantity) }}" min="0"
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="available_quantity" value="{{ old('available_quantity', $livro->available_quantity) }}" min="0"
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all"
                            required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">{{ old('description', $livro->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all">{{ old('notes', $livro->notes) }}</textarea>
                </div>
            </div>
        </div>

        @if($livro->loans()->where('status', 'ativo')->count() > 0)
            <div class="mt-8 bg-yellow-50 text-yellow-800 p-4 rounded-lg text-sm font-medium border border-yellow-200">
                <p class="font-bold text-yellow-900 mb-1">Aviso de Estoque em Uso</p>
                <p>Este livro possui {{ $livro->loans()->where('status', 'ativo')->count() }} empréstimo(s) ativo(s). Tenha cuidado ao alterar as quantidades totais para evitar inconsistências no sistema.</p>
            </div>
        @endif
